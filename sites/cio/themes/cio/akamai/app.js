...
if(jQuery(this).is('.li-btn a')){jQuery(this).hide();}

if($curAnsType=="checkbox"){
	if($curAns.hasClass('noneOf')){
...
//}, $curAnsSkipTimer);
	...
	console.log(totalQus,totalAnsedQus);
		...
		jQuery('.page input[type="checkbox"]').on('change', function(){
			if ($curQus.find("input:checked").length) {$curQus.find('.error-wrap').text('').removeClass('has-error');}else{jQuery(this).prop('checked',true);$curQus.find('.error-wrap').text($curQus.find("input:first").data('validationErrorMsg')).addClass('has-error').delay( 2000 ).fadeOut( 500, function() {
	...
	jQuery('.download.btn').on('click',function(){
			  if (jQuery('#fsi_survey').isValid()) {
				  var _ajaxWFURL = "create.php";
				  $.ajax({
					url : _ajaxWFURL,
					type: "POST",
					data: {Field1: $('.qus1').data('ansVal'),
						   Field2: $('.qus2').data('ansVal'),
						   Field3: $('.qus3').data('ansVal'),
						   Field4: $('.qus4').data('ansVal'),
						   Field5: $('.qus5').data('ansVal'),
						   Field6: $('.qus6').data('ansVal'),
						   Field7: $('.qus7').data('ansVal'),
						   Field8: $('.qus8').data('ansVal'),
						   Field9: $('.qus9').data('ansVal'),
						   Field10: $('.qus10').data('ansVal'),
						   Field11: $('.qus11').data('ansVal'),
						   Field119: jQuery('#total_score').val(),
						   Field120: jQuery('#first_name').val(),
						   Field121: jQuery('#last_name').val(),
						   Field122: jQuery('#company_name').val(),
						   Field123: jQuery('#job_title').val(),
						   Field124: jQuery('#business_email').val(),
						   Field125: jQuery('#telephone_number').val(),
						   Field126: jQuery('input[name='purchase_security_solutions']:checked').val(),
						   Field127: jQuery('input[name='receive_security_briefing']:checked').val()
						  },
...
jQuery(document).ready(function(){
  ...
  //jquery for label change
 // $("#method").change(function() {
   // $('#help-text').text($('option:selected').text()).css("visibility", "hidden");
//}).change();
});
